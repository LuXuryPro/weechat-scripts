...
def notify_show(data, bufferp, uber_empty, tagsn, isdisplayed, ishilight, prefix, message):
    l = tagsn.split(",")
    if ishilight == "1" and not is_displayed(bufferp):
        os.system("xmessage " + "-center " + " -buttons OK:0 "+"-default OK " +message + "&")
    elif "notify_private" in l and not is_displayed(bufferp):
        nick = l[-2].split("_")[-1]
        os.system("xmessage " + "-center " + " -buttons OK:0 "+"-default OK " +nick+ ": " +message + "&")
